...
namespace GenericScoring
{
    ...
    public class ngram_score
    {
        ...
        private static IEnumerable<string> CustomSplitLines(string input)
        {
        ...
        private static IEnumerable<string> CustomSplitSpaces(string input)
        {
            //this one is margenaly better that the second best 2, but makes the resolver (its client much faster), nealy as fast as original.
            var Result = new List<string>();
            var begining = 0;
        ...
        public static bool LoadNgarmDoubleFile(string TrigramDump, string QuadgramDump, char sep = ' ')
        {
            ...
            string[] lines = CustomSplitLines(TrigramDump).ToArray();
            string[] lines2 = CustomSplitLines(QuadgramDump).ToArray();
            ...
            try
            {
                foreach (string line in lines)
                {
                    string key = "";
                    int count = 0;
                    string[] split = CustomSplitSpaces(line).ToArray();
                    key = split[0];
                    count = Convert.ToInt32(split[1]);
